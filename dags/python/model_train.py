...
df.to_csv('df_ch_20_resampled.csv')
print('\n',df.head())
print(len(df))
...
window_size_std = 15
# Calculate the rolling mean and standard deviation
rolling_mean = df_weather.rolling(window=window_size_mean).mean()
rolling_std = df_weather.rolling(window=window_size_std).std()

rolling_std = rolling_std.replace(0, 1)
rolling_mean = rolling_mean.rename(columns={'temperature': 'rolling_mean'})
rolling_std = rolling_std.rename(columns={'temperature': 'rolling_std'})
result = df_weather.join([rolling_mean, rolling_std], how='outer')
z_scores = (result['temperature'] - result['rolling_mean']) / result['rolling_std']
# Threshold for outlier detection
threshold = 3

# Find the outliers using the threshold
outliers = np.abs(z_scores) > threshold
df_weather.to_csv('df_weather_ch_20.csv')

# Remove the outliers from the data
df_weather = df_weather.mask(outliers)
df_weather.to_csv('df_weather_ch_20.csv')'''

# Resampling df_weather to hourly frequency
df_weather = df_weather.resample('H').mean().interpolate(method='linear') 
df_weather.to_csv('df_weather_ch_20_resampled.csv')
print('\n',df_weather.head())
...
df.to_csv('df_parking_and_weather_combined.csv')
